...
export function Navbar() {
  return <header className="site-nav"><Link href="/" className="wordmark">Royal <em>Spice</em></Link><nav className="desktop-nav" aria-label="Main navigation">{navigation.map((item) => <Link key={item.href} href={item.href}>{item.label}</Link>)}</nav><div className="nav-actions"><Link className="text-link" href="/order">Order &amp; Reserve <Arrow /></Link><Link className="menu-trigger" href="/order">Order &amp; Reserve</Link></div></header>;
}
...
export function MobileActionBar() { return <div className="mobile-action-bar"><a href={`tel:${restaurant.contact.phone}`}>Call</a><a href={`https://wa.me/${restaurant.contact.whatsapp}`}>WhatsApp</a><Link href="/order">Order &amp; Reserve</Link></div>; }
